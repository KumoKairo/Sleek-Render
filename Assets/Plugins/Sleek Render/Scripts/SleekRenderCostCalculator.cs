public static class SleekRenderCostCalculator{
	...
	public static int GetBloomCost(bool isEnabled, int yRation){
		if(isEnabled){
			...
			switch(yRation){ // use Horizontal + Vertical Blur values
				case 32:
					return result + 20200;
	...
	public static int GetBloomCost(bool isEnabled, int xRation, int yRation){
		if(isEnabled){
			int result = 13466; // glBlindframebuffer cost
			switch(xRation){ // use Horizontal Blur values
				case 32:
					result += 18000;
					break;
				case 64: // avergae between 32 and 128
			...
			switch(yRation){ // use Vertical Blur values
				case 32:
					return result + 5300;
				case 64: // avergae between 32 and 128
					return result + 20650;
	...
	public static int GetColorizeCost(bool isEnabled){
		if(isEnabled){ // use Compose with colorize - without colorize
			return 130000;
		}
		return 0;
	}
	...
	public static int GetVignetteCost(bool isEnabled){
		if(isEnabled){ // use Precompose with vignette - without vignette
			return 700;
		}
		return 0;
	}
